...
const teenPattiIO = io.of("/teenpatti");
...
const teenPattiWinRecourds = [1, 2, 3, 1, 2, 3, 1, 2];
...
const selectTimeEmitUpdate = async () => {
    const redisValue = await redisClient.get("teenPattiObj");
    ...
    if (currentObject.selectTime <= 0) {
        ...
        const teenPattiCollectionRef = db.collection("teenPatties");
        const query = teenPattiCollectionRef
        ...
        const redisWinRecords = await redisClient.get("teenPattiWinRecourds");
        const winRecords = await JSON.parse(redisWinRecords);

        teenPattiIO.emit("result", {
        ...
        await redisClient.set("teenPattiObj", JSON.stringify(currentObject));
        setTimeout(async () => {
        ...
        // 1.1 get total beted amount - using a more efficient approach
        // Instead of fetching all documents, we'll use a more targeted query
        const coll = db.collection("teenPatties")
            .where("completed", "==", false)
        ...
        // 1.2 get total win amount - using a more efficient approach
        const totalWinCollectionRef = db.collection("teenPatties")
            .where("completed", "==", false)
            .where("status", "==", "win");
        ...
        await redisClient.set("teenPattiObj", JSON.stringify(currentObject));
        teenPattiIO.emit("game", JSON.stringify(currentObject));
...
teenPattiIO.on("connection", (socket) => {
    console.log("A teen patti user connected");
    socket.on("disconnect", () => {
        console.log("A teen patti user disconnected");
    });

    socket.on('hit', (data) => {
        // console.log('Message from Flutter:', data);
        teenPattiIO.emit('teenPattiRemoteHit', { option: data.option, amount: data.amount });
    });

});

module.exports = { teenPattiObj, teenPattiWinRecourds };